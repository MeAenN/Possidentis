...
app.listen(8000);
setInterval(() => {
  http.get(`https://glitch.com/~persistent-curly-blouse`);//Glitch linkinizi doğru şekilde girin!
}, 280000)
              ...
              if (reklam.some(word => msg.content.toLowerCase().includes(word))) {
                try {
                  if (!msg.member.hasPermission("MANAGE_GUILD")) {
                    ...
                    .setDescription("Possidentis  Reklam sistemi, " + `***${msg.guild.name}***` + " adlı sunucunuzda reklam yakaladım.")
...
.setTitle("SES KANALI SİLİNDİ")
			...
			.setFooter(`Possidentis | Mod-Log Sistemi  Kanal ID: ${channel.id}`)
				modlogkanal.send({embed});
			}
	})
...
client.on("userUpdate", async(eski, yeni) => {
  ...
  if(!yeni.username.includes("✶") && client.guilds.get("756817972923727903").members.get(yeni.id).roles.has("756924199775371335")) {
     client.guilds.get("756817972923727903").members.get(yeni.id).removeRole("756924199775371335")
     client.channels.get('757144003106832425').send(`:broken_heart: ${yeni},  ✶Possident tagını çıkardı!`)
    }
     if(yeni.username.includes("✶") && !client.guilds.get("756817972923727903").members.get(yeni.id).roles.has("756924199775371335")) {
      client.channels.get('757144003106832425').send(`:heart: ${yeni}, ✶Possident tagını aldı!`)
      client.guilds.get("756817972923727903").members.get(yeni.id).addRole("756924199775371335")
     }
  }
  })

let oment = require('moment')
require("moment-duration-format");
client.on('guildMemberAdd', async onlycode => {
let sunucu = `756817972923727903` //Katılan fake hesaplara verilecek rolün bulunduğu sunucunun Id'sini yazınız.
let rol = `756929086043062373` //Katılan fake hesaplara verilecek rolün Id'sini yazınız.
let kanal = `757182269344710746` //Katılan kişilerin fake olup olmadığını loglayacak kanalın Id'sini yazınız.
let gün = 7 //Günü giriniz. Bu günden önce giren hesaplar ceza yer.
let hesap = moment.duration(new Date().getTime() - onlycode.user.createdAt.getTime()).format('DD')
if(gün < hesap) return client.channels.get(kanal).send(`**${onlycode.user.tag}** isimli kullanıcı sunucuya giriş yaptı, hesabı **${gün}** günden sonra açıldığı için kullanıcıya **dokunulmadı**.`)
onlycode.addRole(rol)
client.channels.get(kanal).send(`**${onlycode.user.tag}** adlı kullanıcı sunucuya giriş yaptı, hesabı **${gün}** günden önce açıldığı için kullanıcıya \`${client.guilds.get(sunucu).roles.get(rol).name}\` adlı rol **verildi**.`)
});
...
client.on('guildMemberAdd', async member => {
    ...
    let resim = new Discord.Attachment('https://media.giphy.com/media/8njotXALXXNrW/giphy.gif')
    let kişi = member.guild.memberCount
    let kayıtcırol = member.guild.roles.find(kayıtcı => kayıtcı.id = "756900732963389537")
    let kanal = client.channels.get("756843983447261238")
    kanal.send(`Merhaba <@${member.user.id}> POSSİDENTİS'E **HOŞGELDİN!**\n\nSeninle beraber **${kişi}** kişiyiz.\n\nTagımızı alarak bize destek olabilirsin\n\nHesap kuruluş tarihi; **${tarih}** [**${gün}** gün önce]\n\n${kayıtcırol} sizinle ilgilenecektir.`, resim)
})
